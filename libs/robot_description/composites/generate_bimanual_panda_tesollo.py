...
TABLE_DIMS = (1.8288, 0.62865, 0.045)
TABLE_CENTER_Z = 0.9144


def load_alignment_from_handenv_yaml(handenv_yaml_path):
    with open(handenv_yaml_path, "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)

    env_cfg = cfg["env"]
    table_size = tuple(float(x) for x in env_cfg["workstation"]["size"])
    table_center_z = float(env_cfg["workstation"]["pose"][2])

    offset_x = float(env_cfg["robot"]["offset_x"])
    offset_y = float(env_cfg["robot"]["offset_y"])
    offset_z = float(env_cfg["robot"]["offset_z"])

    # Keep z as robot offset (w.r.t. table top). The absolute arm-base z is
    # composed later when writing mount joints:
    # table_center_z + 0.5 * table_size_z + offset_z
    left_pos = (-offset_x, offset_y, offset_z)
    right_pos = (offset_x, offset_y, offset_z)

    return table_size, table_center_z, left_pos, right_pos
...
def add_table_top(bimanual_root,
    ...
    Table center z is explicitly configurable to match simulation world coordinates.

    Args:
        bimanual_root : root Element to append to
        dims          : (length_x, width_y, thickness_z) in metres
        table_center_z: world-frame z of table center
...
"""
    ...
    # Add table_top in aligned world coordinates.
    add_table_top(bimanual_root, dims=TABLE_DIMS, table_center_z=TABLE_CENTER_Z)
    table_size_z = TABLE_DIMS[2]
    ...
    left_mount_z = TABLE_CENTER_Z + 0.5 * table_size_z + left_position[2]
    left_origin = ET.SubElement(left_mount, 'origin',
                                xyz=f"{left_position[0]} {left_position[1]} {left_mount_z}",
                                rpy="0 0 0")
    ...
    right_mount_z = TABLE_CENTER_Z + 0.5 * table_size_z + right_position[2]
    right_origin = ET.SubElement(right_mount, 'origin',
                                 xyz=f"{right_position[0]} {right_position[1]} {right_mount_z}",
...
def main():
    ...
    handenv_yaml = script_dir.parent.parent / 'robot_motion_interface' / 'runtime' / 'HandEnv.yaml'

    # Check if input file exists
    if not single_arm_urdf.exists():
        raise FileNotFoundError(f"Single-arm URDF not found at: {single_arm_urdf}")
    if not handenv_yaml.exists():
        raise FileNotFoundError(f"HandEnv.yaml not found at: {handenv_yaml}")

    print(f"Reading single-arm URDF from: {single_arm_urdf}")
    print(f"Reading alignment from: {handenv_yaml}")
    print(f"Output will be saved to: {output_urdf}")

    table_dims, table_center_z, left_position, right_position = load_alignment_from_handenv_yaml(handenv_yaml)
    global TABLE_DIMS, TABLE_CENTER_Z
