"""
Record RealSense color images after an Enter trigger.

Usage:
    python -m robot_motion_interface.utils.realsense_real_record /path/to/output_dir
    python -m robot_motion_interface.utils.realsense_real_record my_run

Controls:
    Enter   start recording
    q       stop recording and write metadata.json
...
def main():
    parser = argparse.ArgumentParser(description="Record RealSense color images after Enter")
    parser.add_argument(
        "output_dir",
    ...
    print(
        "RealSense color recording ready\n"
        f"  device : {device_name}\n"
        f"  serial : {serial_number}\n"
        f"  color  : {c_intr['width']}x{c_intr['height']}@{fps}fps\n"
        f"  output : {output_dir}\n"
        "Live image is shown in the OpenCV window.\n"
        "Press Enter in this terminal to start recording, input 'q' to stop."
    )
    ...
    try:
        cv2.namedWindow("RealSense color record", cv2.WINDOW_AUTOSIZE)
        ...
        while True:
            ...
            while True:
                ...
                if cmd == "q":
                    ...
                    break
                if cmd != "":
                    print("Input must be empty Enter to start recording, or 'q' to stop.")
                    continue
                if not is_recording:
                    is_recording = True
                    print("[recording] started")
                else:
                    print("[recording] already started; input 'q' to stop.")
            ...
            cv2.imshow("RealSense color record", color)
            if (cv2.waitKey(1) & 0xFF) == ord("q"):
                break

            if is_recording:
                frames.append(save_color_image(output_dir, frame_idx, color, t_capture))
                if frame_idx % 30 == 0:
    ...
    print(f"\nRecording complete: {len(frames)} images saved to {output_dir}")
