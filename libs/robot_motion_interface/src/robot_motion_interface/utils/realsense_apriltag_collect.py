...
# z-offsets in world frame from tag center to bottle landmarks. Assumes the
# bottle stands upright (axis ≈ world +Z), tag glued flat on cap top, tag
# center ≈ cap center in XY.
_CAP_DZ  = 0.0095   # m, cap-center is 9.5 mm below tag center along world -Z
_BODY_DZ = 0.105    # m, body-center is 105 mm below tag center along world -Z

# Schema for the shared (apriltag <-> depth_feat) collection npy.
...
def _load_or_init_dataset(path: Path) -> dict[str, np.ndarray]:
    """Load existing data.npy (pickled dict) or initialise empty schema."""
    if path.exists():
...
def _append_or_override(
    ...
    side is already ahead of partner's side.

    Symmetric alignment between the apriltag half and the depth_feat half of
    data.npy: every "complete sample" has both halves written. Behaviour:
      * N_self == N_partner -> append (start a new sample)
      * N_self <  N_partner -> append (catch up to a row partner already wrote)
      * N_self >  N_partner -> overwrite my last row (I wrote this row last
                                run but partner never matched it — assume user
                                is redoing the same bottle pose)

    Both `my_keys` must grow in lockstep, so this helper writes both at once.
    Returns 'appended' or 'overwritten' for logging.
    """
...
def _save_dataset(path: Path, data: dict[str, np.ndarray]) -> None:
    ...
    # See depth_feat_node_collect._save_dataset for the 0-d object-array trick.
...
DEFAULT_OUT_NPY = PROJECT_ROOT / "data" / "apriltag_depthfeat_collect" / "data.npy"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Rolling mean window (default: {DEFAULT_BUFFER_SIZE}).",
    )
    parser.add_argument(
        "--out-npy",
        type=str,
        default=str(DEFAULT_OUT_NPY),
        help="Output data.npy path (created if absent).",
    )
...
# Rolling buffers — append per frame, mean on Ctrl-C / 'q' → one row to data.npy.
cap_buf  = deque(maxlen=BUFFER_SIZE)
body_buf = deque(maxlen=BUFFER_SIZE)
tag_buf  = deque(maxlen=BUFFER_SIZE)   # raw tag world pos, only for the HUD diff vs cap
...
try:
    while True:
        ...
        if ids is not None and len(ids) > 0:
            ...
            for idx, tag_id in enumerate(id_list):
                ...
                # Buffer rolling samples (only the first matching tag per frame
                # contributes — multiple tags in view shouldn't pollute the same
                # bottle sample).
                if first_pose is None:
                    tag_world  = world_t.astype(np.float32)
                    cap_world  = tag_world - np.array([0.0, 0.0, _CAP_DZ],  dtype=np.float32)
                    body_world = tag_world - np.array([0.0, 0.0, _BODY_DZ], dtype=np.float32)
                    tag_buf.append(tag_world)
                    cap_buf.append(cap_world)
                    body_buf.append(body_world)
        ...
        # HUD — show what would actually be written if you hit Ctrl-C now:
        # buffer-mean of cap/body in world frame. Plus buffer fill status.
        if len(cap_buf) > 0:
            cap_mean  = np.stack(list(cap_buf),  axis=0).mean(axis=0)
            body_mean = np.stack(list(body_buf), axis=0).mean(axis=0)
            hud1 = (
                f"buf={len(cap_buf)}/{BUFFER_SIZE}  "
                f"cap=({cap_mean[0]:+.3f},{cap_mean[1]:+.3f},{cap_mean[2]:+.3f})  "
                f"fps={fps_ema:.1f}"
            )
            hud2 = (
                f"body=({body_mean[0]:+.3f},{body_mean[1]:+.3f},{body_mean[2]:+.3f}) m  "
                f"(world frame, derived from tag - z_offset)"
            )
        else:
            hud1 = f"no detection yet  fps={fps_ema:.1f}"
            hud2 = "(waiting for AprilTag)"
        ...
        legend = f"dict={ARUCO_DICT_NAME}  size={MARKER_SIZE_M*1000:.1f}mm  out={OUT_NPY.name}"
        cv2.putText(vis, legend, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(vis, legend, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)

        cv2.imshow("apriltag_collect", vis)
    ...
    # Dump rolling-buffer mean as one row into data.npy. Both buffers (cap +
    # body) advance together so the same N applies. If nothing was ever
    # detected, save nothing — user can re-run.
    if len(cap_buf) > 0:
        cap_mean  = np.stack(list(cap_buf),  axis=0).mean(axis=0)
        body_mean = np.stack(list(body_buf), axis=0).mean(axis=0)
        data = _load_or_init_dataset(OUT_NPY)
        status = _append_or_override(
            data,
            my_keys=("apriltag_cap", "apriltag_body"),
            partner_keys=("depth_feat_cap", "depth_feat_body"),
            vecs=(cap_mean, body_mean),
        )
        _save_dataset(OUT_NPY, data)
        print(
            f"[saved/{status}] {OUT_NPY}\n"
            f"  apriltag_cap  N={len(data['apriltag_cap'])}  "
            f"mean=({cap_mean[0]:+.4f},{cap_mean[1]:+.4f},{cap_mean[2]:+.4f}) m\n"
            f"  apriltag_body N={len(data['apriltag_body'])} "
            f"mean=({body_mean[0]:+.4f},{body_mean[1]:+.4f},{body_mean[2]:+.4f}) m\n"
            f"  partner depth_feat_cap N={len(data['depth_feat_cap'])}\n"
            f"  averaged over {len(cap_buf)} frames"
        )
    else:
        print("[exit] buffer empty, no sample saved.")
    try:
        rs_pipeline.stop()
    except Exception:
        pass
