...
Generates a sine wave trajectory and executes it on the robot.

Usage:
    python bimanual_traj.py --freq 30.0 --loop
"""
...
def get_args():
    parser = argparse.ArgumentParser(description="Bimanual Sine Wave Trajectory Player")
...
def generate_sine_trajectory(init_pos, duration=5.0, dt=1/30.0, sin_freq=0.5, active_joints=None, amp=0.01):
    """
    Generates a [steps, dim] trajectory. Only specified joints will move.
    
    Args:
        init_pos: [dim] array of initial joint positions (e.g., home position).
    ...
    if active_joints is None:
        ...
        for idx in active_joints:
            if idx < num_joints:
                ...
                raise ValueError(f"Active joint index {idx} out of range for {num_joints} joints.")

    # 2. 生成时间向量 [num_steps, 1]
    t = np.linspace(0, duration, num_steps).reshape(-1, 1)
    
    # 3. 计算基础的正弦波形 [num_steps, 1]
    base_wave = np.sin(2 * np.pi * sin_freq * t)
...
def main():
    ...
    try:
        ...
        trajectory = generate_sine_trajectory(init_pos=home_position, duration=5.0, dt=1/args.freq, sin_freq=0.5, 
                                              active_joints=[2, 3, 10, 14, 17, 18], amp=0.1)
