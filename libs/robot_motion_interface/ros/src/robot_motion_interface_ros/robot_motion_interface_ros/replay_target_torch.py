...
class RunTrajNode(Node):
    def __init__(self):
        ...
        left = data["left_dof_targets"]      # [T, N, 19]
        right = data["right_dof_targets"]    # [T, N, 19]
        ...
        full_targets = torch.cat([left, right], dim=-1)  # [T, N, 38]
        ...
        self.traj = full_targets[:, self.traj_id, :].to(dtype=torch.float32)  # [T, 38]
