"""Policy node that consumes the state-estimator feature from a ROS 2 topic.
...
class DepthFeatPolicyNode(Node):
    def __init__(self):
        ...
        # Destination for the state-estimator topic: body_pos(3) + cap_pos(3) + jar_geom(4),
        # metres, env-local frame (= the sim extero group). The geom slab stays in metres
        # here; _scale_torch normalizes it to [-1, 1] when the obs is composed.
        self.latest_depth_feature = torch.zeros(
            (1, self._EXTERO_DIM), dtype=torch.float32, device=self.device
        )
        self._feat_snapshot_t = torch.empty_like(self.latest_depth_feature)
        self._feat_seen: bool = False
        # Staging buffer: the callback writes into this numpy array and copies the tensor
        # view of it, so no per-message allocation.
        self._feat_cpu = np.zeros(self._EXTERO_DIM, dtype=np.float32)
        self._feat_cpu_t = torch.from_numpy(self._feat_cpu).view(1, -1)
    ...
    def _init_callback_mutex_groups(self) -> None:
        ...
        self.feat_grp = MutuallyExclusiveCallbackGroup()
    ...
    def _declare_parameters(self) -> None:
        ...
        self.declare_parameter("state_estimator_topic", "/state_estimator/extero")
    ...
    def _build_joint_mappings(self) -> None:
        ...
        # Per-class integration scalars: arm joints get the arm_* value, finger joints the
        # finger_* one. Read from the deployed policy's env.yaml (auto-synced with training),
        # NOT mjlab_policy_runtime.yaml which is hand-maintained and drifts stale.
        ...
        def _per_class(names: list[str], arm_val: float, finger_val: float) -> list[float]:
            out = []
            for n in names:
                if re.fullmatch(arm_expr, n):
                    out.append(arm_val)
                elif re.fullmatch(finger_expr, n):
                    out.append(finger_val)
                else:
        ...
        # Target smoothing is per-class too: SmoothedJointPositionAction weights the previous
        # target by arm_target_alpha on the arm and finger_target_alpha on the fingers, so a
        # single scalar here would silently integrate one of the two groups wrong.
        # Policies trained before the per-class split carry a single `target_alpha`; read
        # that rather than assuming a default, so the value always comes from the run that
        # produced the checkpoint.
        _legacy_alpha = _act.get("target_alpha")
        arm_alpha = self._arm_target_alpha = float(
            _act.get("arm_target_alpha", _legacy_alpha)
        )
        finger_alpha = self._finger_target_alpha = float(
            _act.get("finger_target_alpha", _legacy_alpha)
        )

        unprefixed_left = left_panda + left_tesollo
        unprefixed_right = right_panda + right_tesollo
        vel_scale_left = self._to_dev_t(_per_class(unprefixed_left, arm_vel, finger_vel))
        vel_scale_right = self._to_dev_t(_per_class(unprefixed_right, arm_vel, finger_vel))
        alpha_left = self._to_dev_t(_per_class(unprefixed_left, arm_alpha, finger_alpha))
        alpha_right = self._to_dev_t(_per_class(unprefixed_right, arm_alpha, finger_alpha))
        ...
        self.full_alpha_policy = _scatter_full(alpha_left, alpha_right).unsqueeze(0)
    ...
    def _read_hand_link_counts(self) -> None:
        ...
        self.state_estimator_topic = str(self.get_parameter("state_estimator_topic").value)
    ...
    def _init_state_buffers(self) -> None:
        ...
        # State-estimator feature, filled by the topic callback.
    ...
    def _sub_state_estimator_cb(self, msg: JointState) -> None:
        """Latest state-estimator output -> self.latest_depth_feature.

        `position` carries body_pos(3) + cap_pos(3) + jar_geom(4) in metres, env-local, in
        the same order the sim publishes as the extero group -- the geom slab is normalized
        later by _scale_torch, so it must arrive raw.
        """
        n = len(msg.position)
        if n != self._EXTERO_DIM:
            self.get_logger().error(
                f"{self.state_estimator_topic}: got {n} values, expected {self._EXTERO_DIM}"
            )
            return
        with self.feat_lock:
            self._feat_cpu[:] = msg.position
            self.latest_depth_feature.copy_(self._feat_cpu_t, non_blocking=True)
            self._feat_seen = True
    ...
    def _init_pub_sub(self) -> None:
        ...
        self.create_subscription(
            JointState,
            self.state_estimator_topic,
            self._sub_state_estimator_cb,
            HIGH_PERF_QOS,
            callback_group=self.feat_grp,
        )
    ...
    def _policy_update_loop(self) -> None:
        ...
        # 1) Snapshot the latest state-estimator feature.
        if not self._feat_seen:
            self.get_logger().error(
                f"No message received yet on {self.state_estimator_topic}."
            )
            return
        with self.feat_lock:
    ...
    def _build_node_cfg_summary(self) -> dict[str, Any]:
        return {
            ...
            "state_estimator": {
                "shape": list(self.latest_depth_feature.shape),
                "dtype": str(self.latest_depth_feature.dtype),
                "topic": self.state_estimator_topic,
            },
            "runtime": {
                "dt": float(self.dt),
                "action_ema": float(self.action_ema),
                "action_scale": float(self.action_scale),
                "arm_target_alpha": float(self._arm_target_alpha),
                "finger_target_alpha": float(self._finger_target_alpha),
            },
            "obs": {
