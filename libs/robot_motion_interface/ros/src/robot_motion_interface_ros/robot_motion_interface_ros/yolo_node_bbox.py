"""
ROS 2 node: live YOLO bbox-mask depth visualization from a RealSense camera.
...
Pipeline (matches realsense_record.py order — filter THEN align):
    wait_for_frames
        ...
        → YOLOBboxDepthMasker.mask_depth(color, depth)
...
class YoloNodeBBox(Node):
    def __init__(self):
        ...
        padding_ratio = float(self.get_parameter("padding_ratio").value)
        ...
        # ── YOLO masker ─────────────────────────────────────────────────────
        self.masker = YOLOBboxDepthMasker(
            variant=variant,
    ...
    def _loop(self):
        ...
        while self._running and rclpy.ok():
            ...
            # YOLO bbox + mask outside-bbox depth pixels to 0.
            ...
            invalid = d_m <= 0   # bbox-out / no detection
