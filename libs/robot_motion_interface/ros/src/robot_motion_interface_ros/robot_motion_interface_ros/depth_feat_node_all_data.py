...
class DepthFeatNode(Node):
    ...
    def _load_configs(self) -> None:
        ...
        if "T_world_cam" not in ext_cfg:
            raise KeyError(
                ...
                f"realsense_artag_cali.py to regenerate."
            )
        ...
        while self._capture_running and rclpy.ok():
            ...
            if (color_hires_view.shape[0] != self.policy_height or
                ...
                color_bgr = cv2.resize(
                    color_hires_view, (self.policy_width, self.policy_height),
                    interpolation=cv2.INTER_AREA,
                )
                depth_u16 = cv2.resize(
                    depth_hires_view, (self.policy_width, self.policy_height),
                    interpolation=cv2.INTER_NEAREST,
                )
            else:
                # Already at policy resolution — just .copy() to detach from
                # the RealSense-owned buffer (the SDK reuses it next frame).
                color_bgr = color_hires_view.copy()
                depth_u16 = depth_hires_view.copy()

            with self.vision_lock:
                self.latest_color_bgr = color_bgr
                self.latest_depth_u16 = depth_u16
                self.latest_color_hires = color_hires_owned
    ...
    def _setup_apriltag(self) -> None:
        ...
        self._aruco_detector = (
            cv2.aruco.ArucoDetector(self._aruco_dict, params)
            if self._use_new_aruco_api else None
        )
    ...
    def _run_apriltag(self, color_hires: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
        ...
        if self.target_tag_id not in id_list:
            return None
        idx = id_list.index(self.target_tag_id)
    ...
    def _inference_step(self) -> None:
        ...
        with self.vision_lock:
            ...
            color_hires = self.latest_color_hires.copy() if self.latest_color_hires is not None else None
